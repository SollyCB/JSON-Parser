...
static inline json_result json_validate_bool(const char *data) {
    return strcmp(data, "true") && strcmp(data, "false") ? JSON_RESULT_INVALID : JSON_RESULT_SUCCESS;
}
...
static inline json_result json_validate_number(const char *data) {
    ...
    default:
        println("JSON: Invalid number, found %c immediately following it", data[len]);
...
static inline json_result json_skip_over_number(const char *data, uint32 *pos) {
    ...
    if (ret != JSON_RESULT_SUCCESS)
        println("JSON: Failed to validate number - char index %u", *pos);
...
static inline json_result json_skip_over_bool(const char *data, uint32 *pos) {
    ...
    if (ret != JSON_RESULT_SUCCESS)
        println("JSON: Failed to validate bool - char index %u", *pos);
    *pos += 4 + (data[*pos] == 'f');
    return ret;
}
...
static inline json_result json_skip_over_null(const char *data, uint32 *pos) {
    ...
    if (ret != JSON_RESULT_SUCCESS)
        println("JSON: Failed to validate null - char index %u", *pos);
    pos += 4;
    return ret;
}
...
json parse_json(file *f, allocator *alloc, allocation *mem_used)
{
    ...
    case JSON_TYPE_STRING:
        println("JSON: String is an invalid top level item");
        return (json){};
    case JSON_TYPE_NUMBER:
        println("JSON: Number is an invalid top level item");
        return (json){};
    case JSON_TYPE_BOOL:
        println("JSON: Bool is an invalid top level item");
        return (json){};
    case JSON_TYPE_NULL:
        println("JSON: Null is an invalid top level item");
        return (json){};
    default:
        println("JSON: Failed to determine type of top level item");
        return (json){};
    }
}
    ...
    for(uint32 i = 0; i < ret.obj.key_count; ++i) {
        ...
        case JSON_TYPE_STRING:
            ...
            if (ret.obj.values[i].type == JSON_TYPE_INVALID) {
                println("JSON: Got invalid string while parsing object value - char index %u", pos);
                goto fail;
            }
            break;
        case JSON_TYPE_NUMBER:
            ...
            if (ret.obj.values[i].type == JSON_TYPE_INVALID) {
                println("JSON: Got invalid object while parsing object value - char index %u", pos);
                goto fail;
            }
            break;
        case JSON_TYPE_OBJECT:
        ...
        case JSON_TYPE_ARRAY:
            ...
            if (ret.obj.values[i].type == JSON_TYPE_INVALID) {
                println("JSON: Got invalid array while parsing object value - char index %u", pos);
                goto fail;
            }
            break;
        case JSON_TYPE_BOOL:
            ...
            if (ret.obj.values[i].type == JSON_TYPE_INVALID) {
                println("JSON: Got invalid bool while parsing object value - char index %u", pos);
                goto fail;
            }
            break;
        case JSON_TYPE_NULL:
            ...
            if (ret.obj.values[i].type == JSON_TYPE_INVALID) {
                println("JSON: Got invalid null while parsing object value - char index %u", pos);
                goto fail;
            }
            break;
        default:
            println("JSON: Got unrecognisable type parsing object value - char index %u", pos);
            goto fail;
        }
    }
    pos += simd_find_char(data + pos, '}');
...
fail:
    println("JSON: Failed to parse object, Location - ");
...
inline static void json_fill_array_elem(json_array *to, json *from, uint i) {
    ...
    case JSON_TYPE_NULL:
        ...
        break;
    default:
        println("JSON: Invalid type while filling array elem");
    }
}
...
static json json_parse_array(const char *data, uint32 *offset, allocator *alloc)
{
    ...
    if (ret.arr.len == Max_u32) {
        println("JSON: Failed to find array len - char index %u", pos);
        goto fail;
    }
    uint32 i;
    json tmp;
...
fail:
    println("JSON: Failed to parse array - char index %u", pos);
    return (json){};
}
...
static json json_parse_number(const char *data, uint32 *offset, allocator *alloc)
{
    ...
    if (json_validate_number(data + *offset) != JSON_RESULT_SUCCESS) {
        println("JSON: Failed to validate number - char index %u", *offset);
        return (json){};
    }
    json ret;
...
static json json_parse_bool(const char *data, uint32 *offset, allocator *alloc)
{
    ...
    if (json_validate_bool(data + *offset) != JSON_RESULT_SUCCESS) {
        println("JSON: Failed to validate bool - char index %u", *offset);
        return (json){};
    }
    json ret;
...
static json json_parse_null(const char *data, uint32 *offset, allocator *alloc)
{
    ...
    if (json_validate_null(data + *offset) != JSON_RESULT_SUCCESS) {
        println("JSON: Failed to validate null - char index %u", *offset);
        return (json){};
    }
    json ret;
...
static uint32 json_object_count_keys(const char *data)
{
    ...
    while(1) {
        ...
        case JSON_RESULT_END:
            return ret;
        default:
            println("JSON: Failed to find key or find closing '}' - char index %u", pos);
            return Max_u32;
        }
    }
}
static json_parse_fn json_array_get_elem_info(const char *data, json_array *arr, allocator *alloc)
{
    ...
    case JSON_TYPE_NULL:
        ...
        break;
    default:
        println("JSON: Got invalid element type while counting array elems - char index %u", pos);
        arr->len = Max_u32;
        return NULL;
    }
    uint32 i;
    for(i = 0; data[pos] != ']'; ++i) {
        if (skip_fn(data, &pos) != JSON_RESULT_SUCCESS) {
            println("JSON: Failed to skip element %u in array", i);
            arr->len = Max_u32;
            return NULL;
        }
        pos += simd_skip_over_whitespace(data + pos);
...
static json_result json_parser_find_key(const char *data, uint32 *pos)
{
    *pos += simd_skip_over_whitespace(data + *pos);
    ...
    if (data[*pos] == '"')
        if (data[((int)*pos) - 1] == '\\') {
            println("JSON: Found escaped '\"' outside of a key - char index %u", *pos);
            return JSON_RESULT_INVALID;
        } else {
...
static json_result json_skip_over_key_and_value(const char *data, uint32 *pos)
{
    ...
    *pos += simd_skip_over_whitespace(data + *pos);
    if (data[*pos] != ':') {
        println("JSON: Found '%c' in place of ':' when skipping key - char index %u", data[*pos], *pos);
        return JSON_RESULT_INVALID;
    } else {
        *pos += 1;
    }
        ...
        for(j = 0; j < pc0 + pc1; ++j) {
            ...
            if (stack == 0) {
                *pos += i + tz1 + 1;
                ...
                println("JSON: Too many '%c' relative to '%c' while skipping object - char index %u", close, open, *pos + i);
                return JSON_RESULT_INVALID;
            }
...
static void json_print_array(json_array *a, int depth)
{
    ...
    for(int i = 0; i < a->len; ++i) {
        ...
        case JSON_TYPE_NULL:
            ...
            break;
        default:
            println("JSON: Cannot print invalid array elem");
            break;
        }
        println(",");
    }
